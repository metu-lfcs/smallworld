...
(defun construct-sign (lex)
  (make-sign :phon (cadr (assoc 'phon lex))
			 :syn (refresh-vars (cadr (assoc 'syn lex)))
...
(defun match (x y &optional bindings)
  ...
  (cond 
	...
	((assoc x bindings) (match (get-binding x bindings) y bindings))
	((assoc y bindings) (match x (get-binding y bindings) bindings))
	...
	(t
	  (when (and (consp x) (consp y))
		(multiple-value-bind (b2 yes) (match (car x) (car y) bindings)
		  (if yes
...
(defun _apply (func arg)
  (multiple-value-bind (bindings yes) (match (get-input func) arg)
	(if yes 
...
(defun _compose (f g)
  "combine the cats with composition, if possible"
  (multiple-value-bind (bindings yes) (match (get-input f) (get-output g))
	(if yes 
	  (list 
		(cons 'in 
